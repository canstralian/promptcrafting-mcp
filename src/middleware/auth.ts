...
type RateLimitBinding = "RATE_LIMITER" | "BURST_LIMITER";
...
async function verifyJWT(
  ...
  if (payload.iss !== EXPECTED_ISSUER) throw new Error(`JWT issuer mismatch: got '${payload.iss}'`);
  if (payload.aud !== EXPECTED_AUDIENCE) throw new Error(`JWT audience mismatch: got '${payload.aud}'`);
  if (!payload.sub) throw new Error("JWT missing required 'sub' claim");
...
export function requirePermission(
  ...
  return async (c, next) => {
    ...
    if (missing.length > 0) {
      return c.json({ error: "Insufficient permissions", required: permissions, missing }, 403);
    }
...
export function preAuthBurstLimit(
  binding: RateLimitBinding = "BURST_LIMITER",
): MiddlewareHandler<{ Bindings: Env }> {
  return async (c, next) => {
    ...
    if (!success) {
      return c.json({ error: "Rate limit exceeded", retryAfter: 10 }, 429);
    }
...
export function identityRateLimit(
  binding: RateLimitBinding = "RATE_LIMITER",
): MiddlewareHandler<{ Bindings: Env }> {
  return async (c, next) => {
    ...
    const { success } = await c.env[binding].limit({ key: `user:${userId}` });
